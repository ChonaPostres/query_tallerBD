...
declare
	...
	nivel3 cursor (padre integer) for select rubros.id_rubro, rubros.rubro, sum(itemeslicitaciones.totallineaneto) from rubros join productos ON productos.rubro = rubros.id_rubro join itemeslicitaciones ON itemeslicitaciones.codigoproducto = productos.codigoproductos where rubropadre = padre group by rubros.id_rubro, rubros.rubro;
